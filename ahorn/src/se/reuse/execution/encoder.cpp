...
Encoder::Encoder(etfa::Solver &solver) : _old(true), _solver(&solver), _context(nullptr) {}

z3::expr Encoder::encode(const ir::Expression &expression, const Context &context, bool old) {
    _old = old;
    _context = &context;
...
void Encoder::visit(const ir::BinaryExpression &expression) {
    ...
    switch (expression.getBinaryOperator()) {
        ...
        case ir::BinaryExpression::BinaryOperator::ADD: {
            if (left_encoded_expression.is_arith() && right_encoded_expression.is_arith()) {
                z3::expr z3_expression = (left_encoded_expression + right_encoded_expression).simplify();
                _expression_stack.push(z3_expression);
            } else if (left_encoded_expression.is_arith() && right_encoded_expression.is_bool()) {
                z3::expr z3_expression =
                        (left_encoded_expression +
                         z3::ite(right_encoded_expression, _solver->makeIntegerValue(1), _solver->makeIntegerValue(0)))
                                .simplify();
                _expression_stack.push(z3_expression);
            } else if (left_encoded_expression.is_bool() && right_encoded_expression.is_arith()) {
                z3::expr z3_expression =
                        (z3::ite(right_encoded_expression, _solver->makeIntegerValue(1), _solver->makeIntegerValue(0)) +
                         right_encoded_expression)
                                .simplify();
                _expression_stack.push(z3_expression);
            } else {
