...
Executor::Executor(se::etfa::Solver &solver)
    : _old(true), _solver(&solver), _encoder(std::make_unique<Encoder>(*_solver)),
      _flattened_name_to_version(std::map<std::string, unsigned int>()), _context(nullptr),
...
Executor::execute(std::unique_ptr<Context> context, bool old) {
...
void Executor::visit(const ir::AssignmentInstruction &instruction) {
    ...
    z3::expr encoded_expression = _encoder->encode(expression, *_context, _old);
