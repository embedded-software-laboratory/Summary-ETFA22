...
std::unique_ptr<State> State::fork(const Vertex &vertex, const z3::expr &expression) const {
...
std::unique_ptr<State> State::clone() const {
    return std::make_unique<State>(*_vertex, _symbolic_valuations, _path_constraint, _flattened_name_to_version);
}