...
Solver::Solver()
    : _context(std::make_unique<z3::context>()),
      _id_to_uninterpreted_constants(std::map<unsigned int, std::vector<z3::expr>>()), _random_number_generator(108),
...
std::vector<z3::expr> Solver::getUninterpretedConstants(const z3::expr &expression) {
    std::vector<bool> visited;
    /*
    auto it = _id_to_uninterpreted_constants.find(expression.id());
    if (it == _id_to_uninterpreted_constants.end()) {
        std::vector<z3::expr> uninterpreted_constants;
        visit(visited, uninterpreted_constants, expression);
        _id_to_uninterpreted_constants.emplace(expression.id(), uninterpreted_constants);
