...
Merger::Merger(Solver &solver, Executor &executor) : _solver(&solver), _executor(&executor) {}
...
std::unique_ptr<Context> Merger::merge(std::unique_ptr<Context> context_1, std::unique_ptr<Context> context_2) {
    ...
    assert(context_1->getCallStackDepth() == 1 && "Merge only at cycle end!");
    ...
    for (const auto &symbolic_valuation_2 : state_2.getSymbolicValuations()) {
        ...
        if (it == symbolic_valuations.end()) {
            ...
            // TODO: this invariant does not hold anymore in case multiple summaries are applicable -> this leads to
            //  different contexts with the same prefix reaching
        }
    }
    ...
    // maps ids of variables that have been merged to the respective fresh merged variable
    std::map<unsigned int, z3::expr> merged_variables;
    for (auto it = main_cfg.flattenedInterfaceBegin(); it != main_cfg.flattenedInterfaceEnd(); ++it) {
        ...
        unsigned int symbolic_version_1 = state_1.getVersion(flattened_name);
        unsigned int symbolic_version_2 = state_2.getVersion(flattened_name);
        if (symbolic_version_1 != symbolic_version_2) {
            std::string contextualized_name_1 =
                    flattened_name + "_" + std::to_string(symbolic_version_1) + "__" + std::to_string(cycle);
            std::string contextualized_name_2 =
                    flattened_name + "_" + std::to_string(symbolic_version_2) + "__" + std::to_string(cycle);
            ...
            if (z3::eq(symbolic_expression_1, symbolic_expression_2)) {
                ...
                // merged
                z3::expr merged_variable = _solver->makeConstant(contextualized_name, data_type);
                merged_variables.emplace(symbolic_variable_1.id(), merged_variable);
                merged_variables.emplace(symbolic_variable_2.id(), merged_variable);
            }
        } else {
            assert(symbolic_version_1 == symbolic_version_2);
            flattened_name_to_version.emplace(flattened_name, symbolic_version_1);
        }
    }
    // Replace
    /*for (const auto &id_to_merged_variable : merged_variables) {
        std::cout << "Replacing id: " << id_to_merged_variable.first << " with: " << id_to_merged_variable.second
                  << std::endl;
    }*/
    z3::expr_vector substituted_path_constraint_vector(_solver->getContext());
    for (const z3::expr &constraint : path_constraint) {
        z3::expr substituted_constraint = constraint;
        for (const z3::expr &uninterpreted_constant : _solver->getUninterpretedConstants(constraint)) {
            unsigned int id = uninterpreted_constant.id();
            if (merged_variables.count(id)) {
                // constraint contains a merged variable
                z3::expr_vector src(_solver->getContext());
                src.push_back(uninterpreted_constant);
                z3::expr_vector dst(_solver->getContext());
                dst.push_back(merged_variables.at(id));
                substituted_constraint = substituted_constraint.substitute(src, dst);
            } else {
                // do nothing
            }
        }
        /*std::cout << "Substituted: " << constraint.to_string() << " to: " << substituted_constraint.to_string()
                  << std::endl;*/
        substituted_path_constraint_vector.push_back(substituted_constraint);
    }
    z3::expr substituted_path_constraint = z3::mk_and(substituted_path_constraint_vector).simplify();
    std::unique_ptr<State> state = std::make_unique<State>(vertex, std::move(symbolic_valuations),
                                                           std::vector<z3::expr>({substituted_path_constraint}),
                                                           std::move(flattened_name_to_version));
    std::unique_ptr<Context> context = std::make_unique<Context>(cycle, std::move(state), call_stack);
