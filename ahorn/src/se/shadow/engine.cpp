...
std::unique_ptr<Context> Engine::initializeDivergenceExecution(const Cfg &cfg, const TestCase &test_case) {
    ...
    for (const auto &initial_concrete_state_valuation : initial_concrete_state_valuations) {
        if (!_divergence_executor->isWholeProgramInput(initial_concrete_state_valuation.first)) {
            std::string name = initial_concrete_state_valuation.first + "_0__0";
            if (initial_concrete_state_valuation.second.is_int()) {
                path_constraint.push_back(_solver->makeIntegerConstant(name) == initial_concrete_state_valuation.second);
            } else if (initial_concrete_state_valuation.second.is_bool()) {
                path_constraint.push_back(_solver->makeBooleanConstant(name) == initial_concrete_state_valuation.second);
            } else {
                throw std::runtime_error("Invalid z3::expr sort encountered.");
            }
        }
    }
