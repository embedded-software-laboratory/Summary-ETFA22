...
std::unique_ptr<Context> Merger::merge(std::unique_ptr<Context> context_1, std::unique_ptr<Context> context_2) {
    ...
    for (const auto &concrete_valuation_2 : state_2.getConcreteValuations()) {
        ...
        if (concrete_valuations.count(contextualized_name) == 0) {
    ...
    const std::map<std::string, z3::expr> &symbolic_valuations_state_2 = state_2.getSymbolicValuations();
    for (const auto &symbolic_valuation_2 : symbolic_valuations_state_2) {
        std::string contextualized_name = symbolic_valuation_2.first;
        if (symbolic_valuations.count(contextualized_name) == 0) {
            symbolic_valuations.emplace(symbolic_valuation_2);
        } else {
            z3::expr symbolic_expression_1 = symbolic_valuations.at(contextualized_name);
            z3::expr symbolic_expression_2 = symbolic_valuations_state_2.at(contextualized_name);
    ...
    for (auto it = main_cfg.flattenedInterfaceBegin(); it != main_cfg.flattenedInterfaceEnd(); ++it) {
        ...
        if (highest_symbolic_version_1 != highest_symbolic_version_2) {
            ...
            z3::expr symbolic_expression_1 = symbolic_valuations.at(contextualized_name_1);
            z3::expr symbolic_expression_2 = symbolic_valuations_state_2.at(contextualized_name_2);
            ...
            z3::expr concrete_expression_1 = concrete_valuations.at(contextualized_name_1);
