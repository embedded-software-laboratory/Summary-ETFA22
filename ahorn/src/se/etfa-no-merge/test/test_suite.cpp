...
TestSuite::TestSuite(se::etfa::Solver &solver) : _solver(&solver), _test_cases(std::vector<std::unique_ptr<TestCase>>()) {}

void TestSuite::deriveTestCase(const Context &context) {
    auto logger = spdlog::get("ETFA");
    SPDLOG_LOGGER_TRACE(logger, "Deriving test case from context:\n{}", context);
    ...
    for (const z3::expr &constraint : state.getPathConstraint()) {
    ...
    assert(result.first == z3::sat);
    z3::model &model = *result.second;
    std::map<std::string, z3::expr> concrete_valuations = state.getConcreteValuations();
    for (unsigned int i = 0; i < model.size(); ++i) {
        ...
        if (concrete_valuations.count(contextualized_name) != 0) {
            ...
            throw std::runtime_error("Concrete valuation does not exist!");
        }
    }
    SPDLOG_LOGGER_TRACE(logger, "Model satisfying context:\n{}", concrete_valuations);
    ...
    for (auto it = cfg.flattenedInterfaceBegin(); it != cfg.flattenedInterfaceEnd(); ++it) {
        ...
        if (input_variable_it == interface.inputVariablesEnd()) {
            ...
            initial_concrete_state_valuations.emplace(flattened_name, concrete_valuations.at(contextualized_name));
        }
    }
    ...
    for (unsigned int cycle = 0; cycle <= context.getCycle(); ++cycle) {
        ...
        for (auto it = interface.inputVariablesBegin(); it != interface.inputVariablesEnd(); ++it) {
            ...
            cycle_to_concrete_input_valuations.at(cycle).emplace(flattened_name,
                                                                 concrete_valuations.at(contextualized_name));
        }
    }
