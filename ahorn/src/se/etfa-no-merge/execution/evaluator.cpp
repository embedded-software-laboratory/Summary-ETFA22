...
Evaluator::Evaluator(se::etfa::Solver &solver) : _solver(&solver), _context(nullptr) {}
...
void Evaluator::visit(const ir::BinaryExpression &expression) {
    ...
    switch (expression.getBinaryOperator()) {
        ...
        case ir::BinaryExpression::BinaryOperator::ADD: {
            if (left_evaluated_expression.is_arith() && right_evaluated_expression.is_arith()) {
                z3::expr z3_expression = (left_evaluated_expression + right_evaluated_expression).simplify();
                _expression_stack.push(z3_expression);
            } else if (left_evaluated_expression.is_arith() && right_evaluated_expression.is_bool()) {
                z3::expr z3_expression = (left_evaluated_expression + (right_evaluated_expression.is_true()
                                                                               ? _solver->makeIntegerValue(1)
                                                                               : _solver->makeIntegerValue(0)))
                                                 .simplify();
                _expression_stack.push(z3_expression);
            } else if (left_evaluated_expression.is_bool() && right_evaluated_expression.is_arith()) {
                z3::expr z3_expression = ((left_evaluated_expression.is_true() ? _solver->makeIntegerValue(1)
                                                                               : _solver->makeIntegerValue(0)) +
                                          right_evaluated_expression)
                                                 .simplify();
                _expression_stack.push(z3_expression);
            } else {
                ...
                z3::expr z3_expression = ((left_evaluated_expression.is_true() ? _solver->makeIntegerValue(1)
                                                                               : _solver->makeIntegerValue(0)) +
                                          (right_evaluated_expression.is_true() ? _solver->makeIntegerValue(1)
                                                                                : _solver->makeIntegerValue(0)))
                                                 .simplify();
                _expression_stack.push(z3_expression);
            }
