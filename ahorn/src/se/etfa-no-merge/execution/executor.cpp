...
Executor::Executor(se::etfa::Solver &solver, Explorer &explorer, TestSuite &test_suite)
    : _summary_application_checking_time(0), _solver(&solver), _explorer(&explorer), _test_suite(&test_suite),
      _encoder(std::make_unique<Encoder>(*_solver)), _evaluator(std::make_unique<Evaluator>(*_solver)),
...
void Executor::visit(const ir::CallInstruction &instruction) {
    ...
    std::vector<std::pair<std::shared_ptr<summarization::Summary>, z3::model>> applicable_summaries_and_models =
            _summarizer->findConcretelyApplicableSummaries(*_context);
    long elapsed_time = (std::chrono::system_clock::now() - begin_time_point) / 1ms;
    _summary_application_checking_time += elapsed_time;
    /*
    std::cout << "Found " << applicable_summaries_and_models.size() << " applicable summaries in " << elapsed_time
              << "ms." << std::endl;
    */
    if (!applicable_summaries_and_models.empty()) {
        for (const auto &applicable_summary_and_model : applicable_summaries_and_models) {
            ...
            applySummaryConcretely(*applicable_summary_and_model.first, applicable_summary_and_model.second,
                                   *summarized_context);
            SPDLOG_LOGGER_TRACE(logger, "summarized context:\n{}", *summarized_context);
            // update coverage
            for (const z3::expr &assumption_literal : applicable_summary_and_model.first->getAssumptionLiterals()) {
...
void Executor::visit(const ir::IfInstruction &instruction) {
    ...
    if (evaluated_expression.is_true()) {
        tryFork(state, path_constraint, negated_encoded_expression, next_negative_vertex);
    ...
    if (containsUnconstrainedUninterpretedConstant(state, expression)) {
        ...
        switch (result.first) {
            ...
            case z3::sat: {
                ...
                _forked_context = _context->fork(vertex, expression, model);
                break;
            }
            case z3::unknown: {
                // XXX fall-through
...
void Executor::applySummary(const summarization::Summary &summary, Context &context) {
    ...
    // TODO: I see the bug now! We set the version incorrectly, we override "highest" versions...
...
void Executor::applySummaryConcretely(const summarization::Summary &summary, const z3::model &model, Context &context) {
    auto logger = spdlog::get("ETFA");
    SPDLOG_LOGGER_TRACE(logger, "Applying summary concretely:\n{}", summary);
    // 1. go through the symbolic valuations and collect the highest versions of the summarized fb's
    // variables
    // 2. go through the hard constraints of the summary and add for this particular cycle the symbolic
    // valuations by adding to the highest versions of 1. the local versioning -> does this ensure that there
    ...
    for (const auto &hard_constraint : summary.getHardConstraints()) {
        std::string lhs = used_name_to_reversioned_name.at(hard_constraint.first);
        z3::expr rhs = hard_constraint.second;
        z3::expr result = model.eval(rhs, true);
        concrete_valuations.emplace(lhs, result);
        for (const z3::expr &uninterpreted_constant : _solver->getUninterpretedConstants(rhs)) {
            z3::expr_vector src(_solver->getContext());
            src.push_back(uninterpreted_constant);
            z3::expr_vector dst(_solver->getContext());
    ...
    for (const auto &concrete_valuation : concrete_valuations) {
        SPDLOG_LOGGER_TRACE(logger, "{} -> {}", concrete_valuation.first, concrete_valuation.second.to_string());
    }
    for (const auto &symbolic_valuation : symbolic_valuations) {
        SPDLOG_LOGGER_TRACE(logger, "{} -> {}", symbolic_valuation.first, symbolic_valuation.second.to_string());
    }
    ...
    // update state with contents of summary
    for (const auto &concrete_valuation : concrete_valuations) {
        state.setConcreteValuation(concrete_valuation.first, concrete_valuation.second);
    }
    for (const auto &symbolic_valuation : symbolic_valuations) {
        state.setSymbolicValuation(symbolic_valuation.first, symbolic_valuation.second);
    }
