...
Engine::Engine(std::unique_ptr<Configuration> configuration)
    : _configuration(std::move(configuration)),
      ...
      _solver(std::make_unique<se::etfa::Solver>()), _test_suite(std::make_unique<TestSuite>(*_solver)),
    ...
    assert(_configuration->_no_merge.has_value());
    if (*_configuration->_no_merge) {
        _merge_based = false;
    } else {
        _merge_based = true;
    }
...
se::etfa::Solver &Engine::getSolver() const {
    return *_solver;
}
...
void Engine::run(summarization::Summarizer &summarizer, const Cfg &cfg) {
    ...
    while (!isTerminationCriteriaMet()) {
        ...
        if (_merge_based) {
            std::pair<std::unique_ptr<Context>, boost::optional<TerminationCriteria>> result = stepMergeBased();
            ...
            auto result = step();
            long elapsed_time = (std::chrono::system_clock::now() - begin_time_point) / 1ms;
            SPDLOG_LOGGER_TRACE(logger, "Step took {}ms.", elapsed_time);
            if (result.second.has_value()) {
                SPDLOG_LOGGER_TRACE(logger, "Engine encountered local termination.");
                break;
            }
            ...
            std::cout << "Generated " << result.first.size() << " execution contexts." << std::endl;
            for (std::unique_ptr<Context> &succeeding_cycle_context : result.first) {
                _explorer->push(std::move(succeeding_cycle_context));
            }
        }
    }
...
std::pair<std::vector<std::unique_ptr<Context>>, boost::optional<Engine::TerminationCriteria>> Engine::step() {
    auto logger = spdlog::get("ETFA");
    ...
    while (!_explorer->isEmpty()) {
        ...
        if (cfg.getType() == Cfg::Type::PROGRAM) {
            ...
            if (local_termination_criteria.has_value()) {
                return std::make_pair(std::vector<std::unique_ptr<Context>>{}, local_termination_criteria);
            }
        }
    ...
    return std::pair(std::move(succeeding_cycle_contexts), boost::none);
}

std::pair<std::unique_ptr<Context>, boost::optional<Engine::TerminationCriteria>> Engine::stepMergeBased() {
    auto logger = spdlog::get("ETFA");
