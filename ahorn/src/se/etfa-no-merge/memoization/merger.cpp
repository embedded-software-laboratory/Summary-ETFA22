...
Merger::Merger(se::etfa::Solver &solver, Executor &executor) : _solver(&solver), _executor(&executor) {}
...
std::unique_ptr<Context> Merger::merge(std::unique_ptr<Context> context_1, std::unique_ptr<Context> context_2) {
    ...
    // merge concrete valuations
    std::map<std::string, z3::expr> concrete_valuations = state_1.getConcreteValuations();
    for (const auto &concrete_valuation_2 : state_2.getConcreteValuations()) {
        std::string contextualized_name = concrete_valuation_2.first;
        auto it = concrete_valuations.find(contextualized_name);
        if (it == concrete_valuations.end()) {
            concrete_valuations.emplace(concrete_valuation_2);
        }
    }
    ...
    for (auto it = main_cfg.flattenedInterfaceBegin(); it != main_cfg.flattenedInterfaceEnd(); ++it) {
        ...
        if (symbolic_version_1 != symbolic_version_2) {
            ...
            // XXX "ARCADE"-style merging, i.e., use the concrete store of either state
            z3::expr concrete_expression_1 = state_1.getConcreteValuation(contextualized_name_1);
            concrete_valuations.emplace(contextualized_name, concrete_expression_1);
        } else {
    ...
    return std::make_unique<Context>(cycle, std::move(state), call_stack);
}