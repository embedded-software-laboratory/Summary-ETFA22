...
std::unique_ptr<State> State::fork(const Vertex &vertex, const z3::expr &expression, z3::model &model) const {
    ...
    SPDLOG_LOGGER_INFO(logger, "{}", model.to_string());
...
std::unique_ptr<State> State::clone() const {
    return std::make_unique<State>(*_vertex, _concrete_valuations, _symbolic_valuations, _path_constraint,
