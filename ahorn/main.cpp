...
int main(int argc, char *argv[]) {
    ...
    boost::program_options::options_description summarization("Summarization");
    summarization.add_options()("summarization", "performs summarization of function blocks")(
            "change-annotated-program", boost::program_options::value<std::string>(),
            "combined old and new program version")("concolic", "concrete and symbolic valuations")
            ("no-merge", "enumeration without merging")("validity-checking", "performs validity checking");
    ...
    if (vm.count("unreachable-branches")) {
        ...
        if (!unreachable_branches.empty()) {
            ...
            for (const std::string &unreachable_branch : unreachable_branches) {
                ...
                bool branch = unreachable_branch.substr(pos + 1, unreachable_branch.size()) == "tt";
    ...
    if (vm.count("cse")) {
        ...
        engine->run(*cfg);
    } else if (vm.count("summarization")) {
        createLogger("ETFA", level);
        createLogger("Summarization", level);
        createLogger("Reuse", level);
        // transform both program's decisions
        std::shared_ptr<Cfg> transformed_old_cfg;
        {
            auto decision_transformation_pass = std::make_unique<pass::DecisionTransformationPass>();
            transformed_old_cfg = decision_transformation_pass->apply(*cfg);
        }
        if (vm.count("validity-checking")) {
            // symbolic execution
            ...
            // read change-annotated program
            if (vm.count("change-annotated-program")) {
                std::shared_ptr<Cfg> new_cfg = getCfg(vm["change-annotated-program"].as<std::string>());
                ...
                std::cout << "Phase 2 - Reusing summaries across program versions:" << std::endl;
            ...
            if (vm.count("concolic")) {
                if (vm.count("no-merge")) {
                    configuration->_no_merge = true;
                } else {
                    configuration->_no_merge = false;
                }
                // symbolic execution
                auto engine = std::make_unique<se::etfa_no_merge::Engine>(std::move(configuration));
                auto summarization_engine = std::make_unique<se::summarization::Engine>(engine->getSolver());
                ...
                // symbolic execution
                auto engine = std::make_unique<se::etfa::Engine>(std::move(configuration));
                auto summarization_engine = std::make_unique<se::summarization::Engine>(engine->getSolver());
                ...
                if (vm.count("change-annotated-program")) {
                    ...
                    auto reuse_engine = std::make_unique<se::reuse::Engine>(engine->getSolver());
                    // Phase 3 - Reuse summaries across program versions
                    std::cout << "Phase 3 - Reusing summaries across program versions:" << std::endl;
                    se::summarization::Summarizer &summarizer = summarization_engine->getSummarizer();
                    std::shared_ptr<se::summarization::Summarizer> valid_summarizer =
                            reuse_engine->run(*transformed_new_cfg, summarizer);
                    // Phase 4 - Execute program with reusable summaries
                    std::cout << "Phase 4 - Executing new program with reusable summaries:" << std::endl;
