...
TEST_F(TestLibSeAhorn, Benchmark_Dissertation_PPU_ST_Scenario5_Final) {
    using namespace se::ahorn;
    std::shared_ptr<Cfg> cfg = getCfg("../../test/benchmark/dissertation/PPU/ST/Scenario5_Final.st");
    std::cout << cfg->toDot() << std::endl;
    auto configuration = std::make_unique<se::Configuration>();
    configuration->_cycle_bound = 20;
    auto engine = std::make_unique<Engine>(std::move(configuration));
...
TEST_F(TestLibSeAhorn, Benchmark_ETFA_PLCopen_safety_SFAntivalent) {
    using namespace se::ahorn;
