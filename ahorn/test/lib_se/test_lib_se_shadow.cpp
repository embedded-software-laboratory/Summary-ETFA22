...
TEST_F(TestLibSeShadow, Augmentation_Presentation) {
    std::shared_ptr<Cfg> shadow_cfg = getCfg("../../test/benchmark/dissertation/augmentation/presentation/S/S.st");
    std::cout << shadow_cfg->toDot() << std::endl;
    {// shadow execution
        auto configuration = std::make_unique<se::Configuration>();
        configuration->_cycle_bound = 10;
        auto engine = std::make_unique<se::shadow::Engine>(std::move(configuration));
        engine->run(*shadow_cfg, "../../test/benchmark/dissertation/augmentation/presentation/P");
    }
    ASSERT_EQ(0, 1);
}
