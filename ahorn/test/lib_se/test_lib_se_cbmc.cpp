...
TEST_F(TestLibSeCBMC, Benchmark_Dissertation_PLCopen_safety_SFAntivalent) {
    ...
    std::cout << transformed_cfg->toDot() << std::endl;
    auto configuration = std::make_unique<se::Configuration>();
    configuration->_cycle_bound = 5;
    auto engine = std::make_unique<Engine>(std::move(configuration));
    engine->run(*transformed_cfg);
    ASSERT_EQ(0, 1);
}

TEST_F(TestLibSeCBMC, shortened_SFAntivalent_without_call) {
    using namespace se::cbmc;
    std::shared_ptr<Cfg> cfg = getCfg("../../test/lib_se/file/shortened_SFAntivalent_without_call.st");
    auto decision_transformation_pass = std::make_unique<pass::DecisionTransformationPass>();
    std::shared_ptr<Cfg> transformed_cfg = decision_transformation_pass->apply(*cfg);
    std::cout << transformed_cfg->toDot() << std::endl;
    auto configuration = std::make_unique<se::Configuration>();
    ...
    auto engine = std::make_unique<Engine>(std::move(configuration));
    engine->run(*transformed_cfg);ASSERT_EQ(0, 1);
}